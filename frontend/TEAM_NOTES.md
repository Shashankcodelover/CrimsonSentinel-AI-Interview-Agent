...
- The results and chat responses are **NOT fully accurate yet**. They are generated by a local regex/heuristic mock engine (`app/api/interview/route.ts`) to demonstrate the UI. The backend team MUST wire up a real LLM for the evaluations to be context-aware.
